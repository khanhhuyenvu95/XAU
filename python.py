...
# CẤU HÌNH
# =========================
st.set_page_config(page_title="Gold Analyst Pro v8.4", layout="wide")
st.title("🏆 Gold Analyst Pro v8.4 – AI phân tích vàng (Fix RSI + Màu tín hiệu)")
st.caption("Dữ liệu realtime từ Yahoo Finance (fallback Investing.com).")
...
def rsi(close, n=14):
    # ép kiểu 1D series để tránh lỗi ValueError
...
def fetch_data(symbol="XAUUSD=X", interval="1h", period="90d"):
    try:
        ...
        if df.empty:
            st.warning(f"⚠️ Không có dữ liệu cho {symbol}, thử GC=F.")
    ...
    try:
        ...
        try:
            ...
            if isinstance(data, list) and len(data) > 0:
                return round(float(data[-1].get("price", 0)), 2)
        except Exception:
            return None
    return None
...
def analyze(df):
    ...
    if res["macd_cross"] and res["trend"] == "Tăng":
        res["suggest"] = "BUY"
    elif res["rsi"] > 70:
        res["suggest"] = "SELL"
...
if st.button("🔍 Phân tích chuyên sâu"):
    with st.spinner("Đang phân tích..."):
        ...
        if not df.empty:
            ...
            color = "green" if res["suggest"] == "BUY" else "red" if res["suggest"] == "SELL" else "gray"
            st.markdown(f"### 📊 Phân tích kỹ thuật ({selected})")
            st.markdown(f"**Khuyến nghị:** <span style='color:{color};font-size:22px'>{res['suggest']}</span>",
...
st.caption("⚠️ Dữ liệu realtime từ Yahoo Finance, fallback sang Investing.com nếu lỗi.")
